...
这是一个从零重建的摄像头人脸分析项目，核心目标对齐你的产品要求：

- 通过摄像头实时检测人脸
- 对当前人脸进行情绪识别
- 对当前人脸进行眨眼识别并计数

当前版本更像一个可直接演示的产品原型，而不是零散脚本集合。主程序会打开摄像头窗口，实时显示人脸框、情绪标签、情绪置信度、眨眼次数、眼睛开合指标，以及 `py-feat` 提供的 Action Units 简报。
...
使用 `MediaPipe Face Mesh` 从摄像头画面中实时提取人脸关键点。这样我们可以同时拿到：
